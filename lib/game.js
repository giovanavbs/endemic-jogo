...
const PLACE_ACTIONS = ['atacou','explodiu','escondeu','reformou'];
const PERSON_ACTIONS = ['atacou','se infiltrou','matou'];
const CLOSED_PLACES = new Set(['biblioteca','mercado','taverna','igreja','docas','minas','enfermaria']);
...
function placePhrase(action, place) {
  ...
  if (action === 'escondeu') return `se escondeu ${PLACE_PREPOSITIONS[place] || `em ${place}`}`;
  if (action === 'reformou') return `reformou ${object}`;
...
function makeEvent() {
  ...
  if (mode === 0) {
    ...
    let validActions = ['atacou','explodiu','escondeu'];
    const place = pick(PLACES);
    validActions = ['atacou','explodiu','escondeu'];
    if (CLOSED_PLACES.has(place)) validActions.push('reformou');
  ...
  let a = pick(PEOPLE), b = pick(PEOPLE);
  ...
  return [
    {
      ...
      groups: [
        ...
        { label: 'AÇÕES (PARA LUGARES)', items: PLACE_ACTIONS },
    ...
    },
    {
      ...
      groups: [
        { label: 'PESSOAS', items: PEOPLE },
        { label: 'AÇÕES (PARA PESSOAS)', items: PERSON_ACTIONS },
