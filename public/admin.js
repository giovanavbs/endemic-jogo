let timer;let refreshTimer;let authenticated=false;const $=s=>document.querySelector(s);
...
$('#loginBtn').onclick=async()=>{try{await req('/api/admin-login',{method:'POST',body:JSON.stringify({password:$('#password').value})});await enterDashboard()}catch(e){$('#loginMsg').textContent=e.message}};
...
(async function restoreAdminSession(){
  try {
    await req('/api/game?action=admin-state');
