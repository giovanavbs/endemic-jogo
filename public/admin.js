...
$('#loginBtn').onclick=async()=>{try{await req('/api/admin-login',{method:'POST',body:JSON.stringify({password:$('#password').value})});showDashboard();await refresh()}catch(e){$('#loginMsg').textContent=e.message}};
...
if(settingsToggle) settingsToggle.onclick=()=>{
  ...
  const icon=settingsToggle.querySelector('strong');
...
restoreAdminSession();
timer=setInterval(()=>{if(!$('#dashboard').classList.contains('hidden')) refresh()},1500);
